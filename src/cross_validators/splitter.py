...
def train_val_unbalanceBySex(subjects_df,  n_train_subjects, n_val_subjects = 4, seed = 8, Fratio=0.5 ):
    import random
    random.seed(seed)
    all_subjects = subjects_df["subject_id"].tolist()
    ...
    n_females_train = int(n_train_subjects * Fratio)
    n_males_train = int(n_train_subjects * (1-Fratio)) 
    
    random_female = random.sample(female_subjects, n_females_train)
    random_male = random.sample(male_subjects, n_males_train)
    ...
    if n_val_subjects == 0 :
        ...
        # Obtain n_val_subjects * Fratio subjects for val
        n_female_val = int(n_val_subjects * Fratio)
        n_male_val = int(n_val_subjects * (1-Fratio))
        
        random_female = random.sample(female_subjects, n_female_val)
        random_male = random.sample(male_subjects, n_male_val)
